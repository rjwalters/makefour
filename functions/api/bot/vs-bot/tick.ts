 ...
 *
 * This endpoint is designed to be called by a scheduled worker/cron job.
 * It finds all active bot vs bot games that are ready for their next move
 * and advances them.
 */
...
// Maximum games to process per tick to avoid timeout
const MAX_GAMES_PER_TICK = 10

/**
 * POST /api/bot/vs-bot/tick - Advance all ready bot vs bot games
 *
 * Query parameters:
 * - limit: number (default 10, max 20) - max games to process
...
export async function onRequestPost(context: EventContext<Env, any, any>) {
  ...
  try {
    ...
    if (createNew) {
      ...
      const targetActive = 3 // Target number of active bot games

      if (currentActive < targetActive) {
        const gamesToCreate = Math.min(targetActive - currentActive, 2)
